...
async function cutKerf(args: {
  ...
  if (!res.body) throw new Error("/api/strategy returned no body");

  const reader = res.body.getReader();
  const decoder = new TextDecoder();
  let buf = "";
  let kerf: unknown = null;
    ...
    while (true) {
      ...
      while ((idx = buf.indexOf("\n\n")) !== -1) {
        ...
        for (const line of chunk.split("\n")) {
          ...
          if (!isObj(event) || typeof event.type !== "string") continue;
          if (event.type === "kerf") {
            kerf = event.kerf;
  ...
  if (!kerf) throw new Error("Stream ended without a kerf.");
  return kerf;
}
...
async function generateCopy(args: {
  ...
  const json = (await res.json()) as { copy?: unknown; error?: string };
  if (json.error) throw new Error(json.error);
  return json.copy;
}

async function listKerfs(): Promise<unknown> {
  ...
  const json = (await res.json()) as { kerfs?: unknown[]; briefs?: unknown[] };
  return json.kerfs ?? json.briefs ?? [];
}

async function getKerf(args: { id: string }): Promise<unknown> {
...
function parseArgs<T>(schema: z.ZodType<T>, args: unknown, toolName: string): T {
...
const TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        audience: {
          type: "string",
          description:
            "The target audience in plain English (e.g. 'Indie SaaS founders shipping their first $1k MRR')",
        },
        demo: {
          type: "boolean",
          description:
            "If true, returns a canned demo Kerf without running live research. Use for testing.",
          default: false,
        },
      },
  ...
  },
  {
    name: "generate_copy",
    description:
      "Generate platform-ready post copy (hook + caption + visual direction + " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: "object",
          description:
            "The calendar entry you want copy for. Must include day, time, platform, concept_id, post_idea, rationale.",
        },
        demo: {
          type: "boolean",
  ...
  },
  {
    name: "list_kerfs",
    description:
      "List the caller's saved Kerfs (newest first, max 50). Returns an array " +
  ...
  // ---- Deprecated v0.1 aliases (still routed to same impls) ----
  {
    name: "generate_brief",
    description:
      "(Deprecated v0.1 alias for cut_kerf. Same args, same return. Use cut_kerf.)",
    inputSchema: {
      type: "object",
      properties: {
        url: { type: "string" },
...
const server = new Server(
  ...
  { name: "kerfbox", version: PKG.version },
  { capabilities: { tools: {} } }
);
