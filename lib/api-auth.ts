 ...
 * whole thing is parsed. An attacker with one valid key (or an anonymous
 * caller inside the IP bucket) could POST hundreds of MB and force a full
 * parse + schema walk on the expensive inference routes. We cap on the
 * declared Content-Length first (cheap, before reading the stream).
 *
